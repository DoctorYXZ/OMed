...
# Define the folder path for the output 224*224 pixel patches.
sub_output_folder_path = 'C:\\path\\to\\save your 224patch files'
        ...
        for y in range(0, height, 224):
            for x in range(0, width, 224):
                if (x + 224 <= width) and (y + 224 <= height):
                    ...
                    total_area = 224 * 224  # Total number of pixels in the current patch
                    tissue_ratio = tissue_area / total_area  # Calculate the proportion of tissue

                    # If the proportion exceeds 90%, save the patch
                    # This threshold can be adjusted according to your needs
                    if tissue_ratio > 0.90:
